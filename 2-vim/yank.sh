...
raw_tty=$(ps ax | grep $$ | awk '{ print $2 }')
pane_active_tty=$(tmux list-panes -F "#{pane_active} #{pane_tty}" 2>/dev/null | awk '$1=="1" { print $2 }')
target_tty="${SSH_TTY:-${pane_active_tty:-$raw_tty}}"
